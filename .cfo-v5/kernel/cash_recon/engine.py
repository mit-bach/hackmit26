...
def requires_sign_off(amount: float, disposition: str) -> bool:
    """Material matches need a second Bot. An unexplained line always does.

    The threshold is the company close materiality on this Computer.
    """
    ...
    root = os.environ.get("HARNESS_COMPUTER")
    if root:
        path = Path(root) / "data" / "close" / "materiality.json"
        if path.is_file():
            try:
                raw = json.loads(path.read_text())
                materiality = float(raw.get("close_materiality_dollars") or materiality)
            except (OSError, TypeError, ValueError):
                materiality = 1000.0
    return abs(float(amount)) >= materiality
