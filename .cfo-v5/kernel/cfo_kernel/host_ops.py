...
def kernel_bill_facts(args: dict) -> dict:
    from cfo_kernel.facts import bill_facts

    invoice_id = str(args.get("invoice_id") or "")
    if not invoice_id:
        raise ValueError("kernel.bill_facts requires args.invoice_id")
    return bill_facts(invoice_id)


def kernel_cash_signoff(args: dict) -> dict:
    from cfo_kernel.facts import cash_requires_sign_off

    return cash_requires_sign_off(float(args.get("amount") or 0), str(args.get("disposition") or ""))


HOST_DISPATCH = {
    "kernel.health": (kernel_health, "read"),
    "kernel.bill_facts": (kernel_bill_facts, "read"),
    "kernel.cash_signoff": (kernel_cash_signoff, "read"),
