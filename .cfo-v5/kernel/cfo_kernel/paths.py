"""Point Kernel DATA_DIR and RUNS_DIR at one Computer tree.
    ...
    HARNESS_COMPUTER=<computer>
    CFO_EVAL_PHASE=operational
    PYTHONPATH=<computer>/kernel

    DATA_DIR = $HARNESS_COMPUTER/data
    RUNS     = $HARNESS_COMPUTER/runs

``data/`` is the seed tree copied onto the Computer. ``runs/`` is the
mutable overlay. Idempotency and kernel logs live under
``$HARNESS_COMPUTER/cfo/``. Every path comes from ``HARNESS_COMPUTER``.
The Sidecar remaps every ``configure_*`` helper onto that Computer at start.
"""
...
def attach_computer(root: Path) -> Computer:
    ...
    if not data.exists():
        raise FileNotFoundError(
            ...
            "Copy the world pack onto computer/data. See cfo_kernel.paths."
        )
