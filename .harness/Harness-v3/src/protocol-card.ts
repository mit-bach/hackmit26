/**
 * Tool protocol for bound Bots. Pi does not persist the system prompt in
...
import { botSystemPath, piSessionDir, protocolCardPath } from "./paths.ts";

export const PROTOCOL_WAKE_FOOTER =
  "Tools: assistant text → Operator only. ask_bot / bot_ask → pair thread (send or reply). message_operator → Operator during a peer wake. Never put a teammate answer only in assistant text.";

export const PROTOCOL_CARD = `# How to use tools on this Computer
...
export function persistProtocolCard(computerRoot: string, botId?: string): void {
  if (!loadLayout(computerRoot).protocolFiles) {
    return;
  }
  ...
  const systemPath = botSystemPath(computerRoot, botId);
  ensureDir(dirname(systemPath));
  writeFileSync(systemPath, PROTOCOL_CARD);
  const sessionDir = piSessionDir(computerRoot, botId);
  ensureDir(sessionDir);
  writeFileSync(join(sessionDir, "SYSTEM.md"), PROTOCOL_CARD);
}
