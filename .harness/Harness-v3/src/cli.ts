...
import { applyServeComputerEnv } from "./sidecar.ts";
import { startServer } from "./server/http.ts";
import { resolveOfficeLaunch, resolveServeComputer } from "./server/office-instances.ts";
...
export async function runCli(argv: string[]): Promise<void> {
  const [cmd, ...rest] = argv;
  const computerFlag = takeOption(rest, "--computer") ?? process.env.HARNESS_COMPUTER;
  const computerRoot = computerFlag ? resolve(computerFlag) : resolveOfficeLaunch(process.cwd());

  if (!cmd || cmd === "help" || cmd === "--help") {
    process.stdout.write(`harness — named Bots on a shared Computer

Commands:
  serve [--port N] [--no-workers] [--lazy] [--fake] [--eager]
...
`);
  ...
  if (cmd === "serve") {
    const servedRoot = resolveServeComputer(computerRoot);
    applyServeComputerEnv(servedRoot);
    initComputer(servedRoot);
    const client = loadClientRuntime(servedRoot);
    const home = loadOperatorConfig();
    const merged = overlayOperatorConfig(servedRoot, home, client);
    ...
    process.stdout.write(`harness listening ${started.url} computer=${servedRoot}\n`);
