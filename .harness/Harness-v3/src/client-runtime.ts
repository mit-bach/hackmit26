...
function parseSidecar(value: unknown): ClientSidecarSpec | undefined {
  ...
  const portFile = typeof value.portFile === "string" ? value.portFile.trim() : "";
  if (portFile.length === 0 || typeof value.command !== "string" || value.command.trim().length === 0) {
    return undefined;
  }
...
export function parseClientRuntime(value: unknown, _computerRoot: string): ClientRuntime {
  if (!isRecord(value)) {
    return {
      extraExtensions: [],
      clientSkills: false,
  ...
  const sidecar = value.sidecar === false ? undefined : parseSidecar(value.sidecar);
  return {
...
export function applyClientEnv(
  ...
  if (runtime.evalPhase) {
    next.HARNESS_EVAL_PHASE = runtime.evalPhase;
  }
  return next;
}
