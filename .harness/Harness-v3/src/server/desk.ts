...
function promptPreviewBody(computerRoot: string, bot: BotRecord, roster: Roster): Record<string, unknown> {
  const assembled = assembleContext(computerRoot, bot, roster);
  const sections = [
    {
      id: "office",
      label: "Office",
      text: assembled.office.text,
      bytes: Buffer.byteLength(assembled.office.text),
    },
    {
      id: "bot",
      label: "Bot",
      text: assembled.bot.text,
      bytes: Buffer.byteLength(assembled.bot.text),
    },
  ];
  ...
  return {
    ...
    note: "Office system plus this Bot's inlined instructions. Memory and recent work are not in the prefix.",
  };
}
