...
export function listComputerTree(computerRoot: string, from = "workspace"): TreeEntry[] {
  ...
  const walk = (abs: string, depth: number): void => {
    ...
    for (const ent of entries) {
      ...
      if (SKIP_DIR.has(ent.name) || ent.name === "data" || ent.name === "pi-session" || ent.name.endsWith(".lock") || ent.name.includes(".tmp.")) {
        continue;
      }
      if (ent.name === "pi-rpc.jsonl" || ent.name === "pi-runtime.jsonl") {
        continue;
      }
