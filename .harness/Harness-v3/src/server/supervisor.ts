...
import { jailTemplate, seatbeltProfilePath } from "../seatbelt.ts";
import { deskRel, loadLayout } from "../layout.ts";
...
function spawnBot(
  ...
  const jailed = jailTemplate(root) !== undefined;
  const cwd = jailed ? join(root, deskRel(layout, slug)) : root;
  const childEnv = {
    ...env,
    HARNESS_BOT: slug,
  ...
  if (jailed) {
    if (process.platform !== "darwin") {
      throw new Error("this Computer's jail requires sandbox-exec on darwin");
    }
