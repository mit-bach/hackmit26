...
test("assembleContext inlines office, bot, profile, and skill text", () => {
  ...
  assert.equal(first.systemSuffix.includes("Profile prepare"), false);
  ...
  assert.equal(readActiveProfile(root, "bot_ap", wake), "");
  ...
  assert.equal(readActiveProfile(root, "bot_ap", headerWake), "");
  assert.equal(assembleContext(root, bot(), roster(), headerWake).systemSuffix.includes("Profile investigate"), false);
});
