...
export function ComputerPanel({
  ...
  const files = tree.filter((row) => row.type === "file");
  const dirs = tree.filter((row) => row.type === "dir");
    ...
    >
      ...
      <div className="flex min-h-0 flex-1">
        <div className="w-[42%] overflow-y-auto border-r border-hairline/40 text-[12px]">
          ...
          {dirs.map((row) => (
            <div key={`d-${row.path}`} className="flex items-center gap-1.5 px-3 py-1 text-ink-secondary">
              <Folder size={12} />
          ...
          {files.map((row) => (
            <button
              ...
              className={cn(
                "flex w-full items-center gap-1.5 px-3 py-1 text-left hover:bg-raised/50",
                selected === row.path ? "bg-raised text-ink" : "text-ink",
              )}
            >
              <FileText size={12} className="shrink-0 text-ink-secondary" />
              <span className="truncate">{row.path}</span>
