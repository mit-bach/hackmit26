...
function Shell(): React.ReactElement {
  ...
  const bot = group ? undefined : (state.bots.find((item) => item.id === state.selectedId) ?? state.bots[0]);
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        {state.activeView === "protocol" ? (
          ...
          <main className="flex h-full min-w-0 flex-1 flex-col items-center justify-center gap-3 bg-app text-ink-secondary">
            {state.connected ? null : <Loader2 size={20} className="animate-spin" />}
            <div className="text-[14px]">
              {state.connected ? "No Bots on this Computer." : "Connecting to the Harness host…"}
            </div>
            {state.connected ? (
              <button
                type="button"
              ...
              <div className="text-[12px]">
                Start it with <code className="rounded bg-raised px-1.5 py-0.5">npm run serve -- --computer examples/protocol-floor --fake</code>
              </div>
            )}
          </main>
        )}
        ...
        {state.computerOpen && state.activeView === "chat" && (
          <ComputerPanel key={`computer:${bot?.id ?? "floor"}`} bot={bot} />
        )}
        {state.inspectorOpen && bot && <InspectorPanel key={bot.threadId} bot={bot} />}
