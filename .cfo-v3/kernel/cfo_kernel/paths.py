"""Point Kernel DATA_DIR and RUNS_DIR at one Computer tree.
    ...
    HARNESS_COMPUTER=/Users/dominikbach/olympus/hackmit/hackmit26/.cfo-v3
...
``data/`` is the seed tree. The office Computer mounts
``.cfo-v3/world/maximor`` (company pack + Stripe sim). Do not mount
kernel unit-test fixtures.
``runs/`` is mutable overlay / cases / packets / traces.
Idempotency and kernel logs live under ``$HARNESS_COMPUTER/cfo/``.

The Sidecar remaps every ``configure_*`` helper onto the Computer at start.
"""
...
DEFAULT_COMPUTER = Path("/Users/dominikbach/olympus/hackmit/hackmit26/.cfo-v3")


def _repo_root() -> Path:
    """Repo root. Discovery uses .harness only."""
    for parent in [KERNEL_ROOT, *KERNEL_ROOT.parents]:
...
def attach_computer(root: Path) -> Computer:
    ...
    if not data.exists():
        raise FileNotFoundError(
            ...
            "Mount .cfo-v3/world/maximor at computer/data. See cfo_kernel.paths."
        )
