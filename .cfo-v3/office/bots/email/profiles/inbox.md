...
The finance inbox transport named this Bot with Profile `inbox`. Wake text names a delivered message id. This is the Gmail-like front door into Canonical AP.

## Do

1. List the inbox threads, then open the message. Do not invent headers, bodies, or attachments. Mail that a counterparty just delivered is in this inbox. The seeded candidate list is a different store. Classify the new vendor, customer, or bank notes, not that list.
...
5. Write the Computer path. Await the Handle.
