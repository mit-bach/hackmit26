...
Wake text names a packet at `workspace/close/packets/<period>.json`. Kernel traces stay under `runs/month_end/`. The pack a person opens is HTML at `workspace/close/packets/<period>.html`, with the gate result in it. September stays not CLOSED while `$12.40` is open. That HTML is emitted only by kernel close-host code. `.cfo-v3/kernel` is not on this Computer, so this desk does not claim the HTML file exists here.
