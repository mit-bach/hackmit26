...
import { pairsFromCashPayload, PairingLanes, NORTHSTAR_TXN } from "../components/boards/PairingLanes";
...
export default function Cash(): JSX.Element {
  ...
  const pairs = pairsFromCashPayload({
    matches,
    featuredCases: data?.featured_cases,
  });
  const selectedPair =
    pairs.find((pair) =>
      focus === "unexplained"
        ? pair.kind === "unexplained"
        : focus === "fee_netted"
          ? pair.matchType === "FEE_NETTED"
          : pair.kind === "matched" || pair.matchType === "GROUPED_MATCH"
    ) || pairs[0];
  ...
  return (
    <DemoLayout
      ...
      extra={
        <PairingLanes
          pairs={pairs}
          selectedId={selectedPair?.id ?? NORTHSTAR_TXN}
          onSelect={(id) => {
            const pair = pairs.find((row) => row.id === id);
            if (!pair) {
              return;
            }
            if (pair.kind === "unexplained") {
              setFocus("unexplained");
              return;
            }
            if (pair.matchType === "FEE_NETTED") {
              setFocus("fee_netted");
              return;
            }
            setFocus("grouped");
