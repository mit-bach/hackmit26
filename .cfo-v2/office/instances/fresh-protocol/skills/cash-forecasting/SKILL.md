...
Apply when Bot `story` Profile `forecast` (Cash Forecast Agent) reviews a `CashForecastSnapshot`. Weekly totals have already been rolled forward from forecast lines. Forecast Reviewer Agent is a Kernel constructor only, not a Bot.
