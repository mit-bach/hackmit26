...
4. If Kernel inbox already holds the message, wear Profile `triage` on a new Wake. Do not union Grants.
5. If it is a remittance or payment confirmation, `candidate` is null. Destination is `apply`.
6. If it is not an invoice, `candidate` is null. Short factual reason. No chain-of-thought dump.
7. Write the Computer path. `bot_send_prompt` to `ap` (bill) or `apply` (remittance). Await the Handle.
