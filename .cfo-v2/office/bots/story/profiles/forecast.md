...
2. If `forecast_id` is set, call `reporting.tools.get_forecast_snapshot`. Else call `reporting.tools.get_cash_forecast`.
3. Call `reporting.tools.get_forecast_checks` on the snapshot id. If `errors` is non-empty, write `INSUFFICIENT`. Stop.
4. Interpret timing risk, held AP, and low-confidence AR from the snapshot. Cite `forecast_id` and source ids.
5. Treat held AP as uncommitted. Do not speak as if those invoices will be paid.
6. Keep low-confidence AR on the forecast. Label it uncertain. Do not invent inflows.
7. Do not change amounts. Do not rebuild week totals. Python already rolled 13 weeks.
8. If `lock_status` is not `CLOSED`, prefix the narrative with `UNLOCKED`.
