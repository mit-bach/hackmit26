...
Apply when Bot `story` Profile `forecast-miss` (Forecast Variance Agent) receives a `ForecastVarianceExplanation`. Python has already compared the immutable snapshot with later actuals. Forecast Reviewer Agent is a Kernel constructor only, not a Bot.
