...
def configure_paths(directory: Path) -> None:
    ...
    STATE_DIR = directory
    from bs_recon.store import configure_paths as configure_bs
    from close.cash_overlay import configure_paths as configure_overlays
    from close.context import configure_paths as configure_context
    from close.ledger import configure_paths as configure_ledger
    ...
    configure_context(directory)
    configure_ledger(directory)
    configure_prepaid(directory)
    configure_assets(directory)
    configure_bs(directory)
