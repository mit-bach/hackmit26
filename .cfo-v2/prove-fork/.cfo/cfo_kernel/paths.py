...
Choice (prove-fork copy; never the Golden live office):
...
``data/`` is the seed tree. This Computer mounts
``.cfo-v2/prove-fork/world/maximor``. Do not mount the Golden pack or
...
# This file lives at prove-fork/.cfo/cfo_kernel. Parent is the fork office, not the git repo.
REPO_ROOT = KERNEL_ROOT.parent
DEFAULT_COMPUTER = REPO_ROOT / "computer"
...
def attach_computer(root: Path) -> Computer:
    ...
    if not data.exists():
        raise FileNotFoundError(
            ...
            "Symlink prove-fork/world/maximor to computer/data. See cfo_kernel.paths."
        )
