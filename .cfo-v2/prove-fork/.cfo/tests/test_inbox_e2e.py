...
def test_e2e_mixed_inbox_record_counts():
    ...
    assert "PROMPT_INJECTION" in results[-1].trace.reason_codes
    assert statuses["MSG-INBOX-014"] == "REJECTED"
    assert classes["MSG-INBOX-014"] != "VENDOR_INVOICE" or results[-1].invoice_id is None

    assert len(created) == 2  # clean + clarification; injection must not mint
    assert len(created_invoice_ids()) == 2
