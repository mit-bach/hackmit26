...
def _run_auditor_agent(run: AuditRun):
    ...
    return interpretation, report, [usage_from_agent(auditor_agent), usage_from_agent(audit_report_agent)]
...
def run_audit(
    ...
    *,
    ...
    if use_agent:
        interpretation, report, agents = _run_auditor_agent(run)
        ...
        run.agents = [usage_from_agent(auditor_agent), usage_from_agent(audit_report_agent)]
    if persist:
        save_run(run)
    return run
