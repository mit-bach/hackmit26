...
Final close review for a period. A file the host already wrote, including `gates.json` and any pack under `workspace/` or `runs/`, is not the review. Open the finance API gate and the finance API pack for that period before you decide. Pi `read` and `bash` over those files fail the review even if the conclusion happens to be right.
