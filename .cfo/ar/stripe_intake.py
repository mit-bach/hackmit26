...
def _control():
    try:
        from simulations.stripe.control import get_control
...
def _cash_agent_record(trace, *, llm_called: bool, decision_fn_name: str, agent_invoked: bool) -> dict:
    facts = trace.facts
    tools = ["get_cash_application_facts"]
    if facts and facts.identified_customer_id:
        tools.append("get_ar_customer")
        if facts.precedents:
            tools.append("get_ar_precedents")
    agents = ["Cash Application Agent"]
    skills = ["cash-application"]
    if trace.reviewer is not None:
        agents.append("Cash Application Reviewer")
    validations = ["validate_proposal"]
    if trace.reviewer is not None:
        validations.append("needs_reviewer")
    if trace.posted:
        validations.append("post_application_balanced_journal")
    return _record(
        "cash_apply",
        {
            "agent_invoked": agent_invoked,
            "agents": agents,
            "decision_requested": "Choose AUTO_APPLY, HUMAN_REVIEW, or UNAPPLIED among Python candidates.",
            "evidence": list(facts.facts) if facts else [],
            "candidates": [
                {
                    "candidate_id": item.candidate_id,
                    "match_type": item.match_type,
                    "invoice_ids": [row.invoice_id for row in item.applications],
                }
                for item in (facts.candidates if facts else [])
            ],
            "skills": skills,
            "tools_called": tools,
            "llm_called": llm_called,
            "decision_fn": decision_fn_name,
            "final_decision": trace.final.decision if trace.final else None,
...
def apply_stripe_payment(obj: dict, *, event_type: str, live: bool = False) -> dict:
    ...
    control = _control()
    decision_fn = None
    decision_fn_name = "policy_cash_decision"
    llm_called = bool(live)
    agent_invoked = False
    if control is not None:
        if control.decision_mode == "strict":
            from ar.cash import strict_deterministic_cash_decision

            decision_fn = strict_deterministic_cash_decision
            decision_fn_name = "strict_deterministic_cash_decision"
            llm_called = False
        elif control.decision_mode == "agentic" and control.use_llm:
            live = True
            llm_called = True
            agent_invoked = True
            decision_fn_name = "cash_application_agent"
        elif control.decision_mode == "agentic":
            decision_fn_name = "cash_application_agent_interface"
            llm_called = False
            agent_invoked = True
    trace = run_cash_apply(payment.payment_id, live=live, persist=True, decision_fn=decision_fn)
    invoice_ids = [item.invoice_id for item in (trace.final.applications if trace.final else [])]
    ...
    agent_record = _cash_agent_record(
        trace,
        llm_called=llm_called,
        decision_fn_name=decision_fn_name,
        agent_invoked=agent_invoked,
    )
    return {
        "status": "processed",
...
def apply_stripe_refund(obj: dict, *, event_type: str) -> dict:
    ...
    result["agent_record"] = _record(
        "refund_link",
        {
            "agent_invoked": False,
            "agents": [],
            "decision_requested": "Link refund to original Stripe charge/payment and reverse AR by cents.",
            "evidence": [
                f"refund_id={refund_id or charge_id}",
                f"charge_id={charge_id}",
                f"payment_id={payment.payment_id if payment else None}",
            ],
            "skills": [],
            "tools_called": ["find_payment_for_stripe"],
            "llm_called": False,
            "decision_fn": "find_payment_for_stripe",
            "final_decision": result.get("status"),
            "validations": ["refund_cents", "duplicate_stripe_refund_id"],
            "downstream": {"invoice_ids": invoice_ids, "amount": result["amount"]},
        },
    )
    return result
...
def apply_stripe_dispute(obj: dict, *, event_type: str) -> dict:
    ...
    result["agent_record"] = _record(
        "dispute_link",
        {
            "agent_invoked": False,
            "agents": [],
            "decision_requested": "Link dispute to original Stripe charge/payment; do not treat later payout as unexplained.",
            "evidence": [
            ...
            "decision_fn": "find_payment_for_stripe",
            "final_decision": result.get("status"),
            "validations": ["dispute_cents", "duplicate_stripe_dispute_id"],
            "downstream": {"invoice_ids": invoice_ids, "amount": result["amount"], "fee_amount": result["fee_amount"]},
        },
    )
    return result
