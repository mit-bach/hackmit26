...
def format_summary(ctx: CompanyScenarioContext, *, data_root: Path | None = None) -> str:
    ...
    return "\n".join(
        [
            ...
            f"  historical AP register: {len(ctx.historical_ap_register)}",
            f"  documents: {len(ctx.document_texts)}",
            f"  AR invoices: {len(ctx.ar_invoices)}",
            f"  planted scenarios: {len(ap_ids)}",
            "",
            "Cash",
            f"  bank transactions: {len(ctx.bank_transactions)}",
            f"  bank history: {len(ctx.bank_history)}",
            f"  processor txs: {len(ctx.processor_transactions)}",
