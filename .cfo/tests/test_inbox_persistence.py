...
def test_inbox_invoice_survives_in_process_overlay_reload():
    ...
    from tools import unload_runtime_invoices
    from inbox.store import reset_inbox_state

    reset_inbox_state()
    unload_runtime_invoices()
...
def _process_a() -> None:
    ...
    if os.environ.get("CFO_INGEST_STATE_DIR"):
        configure_registry(Path(os.environ["CFO_INGEST_STATE_DIR"]))
    result = handoff(spec_clean_attachment(), persist=True)
    persist_state()
    payload = {
        "invoice_id": result.invoice_id,
