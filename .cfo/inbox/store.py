...
def persist_state() -> Path:
    RUNS_DIR.mkdir(parents=True, exist_ok=True)
    payload = {
        "messages": [item.model_dump(mode="json") for item in all_messages()],
        "outcomes": {key: item.model_dump(mode="json") for key, item in all_outcomes().items()},
        "invoice_ids": created_invoice_ids(),
        "invoices": [item.model_dump(mode="json") for item in created_invoices()],
        "canonicals": [item.model_dump(mode="json") for item in _canonicals.values()],
        "notices": list(_notices),
        "traces": [item.model_dump(mode="json") for item in _traces.values()],
    }
