...
| financial-variance-analysis | Explain period or budget metric movements from Python account and transaction attribution | Bot `story` Profile `flux` (Variance Analysis Agent). Reporting Reviewer Agent remains a Kernel constructor, not a Bot. | `skills/financial-variance-analysis/SKILL.md` | New | board-financial-reporting |
| cash-forecasting | Interpret a Python 13-week cash forecast built from AP, AR, and payroll lines | Bot `story` Profile `forecast` (Cash Forecast Agent). Forecast Reviewer Agent remains a Kernel constructor, not a Bot. | `skills/cash-forecasting/SKILL.md` | New | ar-cash-forecasting |
| ar-cash-forecasting | Interpret Python receivable collection dates, promises, and dispute flags inside a cash forecast | Bot `story` Profile `forecast` (Cash Forecast Agent). | `skills/ar-cash-forecasting/SKILL.md` | New | cash-forecasting |
| forecast-vs-actual-interpretation | Explain a cash forecast miss using Python timing, amount, new, and residual contributors | Bot `story` Profile `forecast-miss` (Forecast Variance Agent). Forecast Reviewer Agent remains a Kernel constructor, not a Bot. | `skills/forecast-vs-actual-interpretation/SKILL.md` | New | cash-forecasting |
| board-financial-reporting | Write a compact board narrative from Python statements, variances, and the cash forecast | Bot `story` Profile `board` (Board Reporting Agent). `audit` samples the pack. Reporting Reviewer Agent remains a Kernel constructor, not a Bot. | `skills/board-financial-reporting/SKILL.md` | New | financial-variance-analysis |
...
| prior-period-precedent | Retrieve prior-period decisions, compare them to current evidence, and reuse a treatment only when the current facts support it | Exception Investigator, AP Reviewer, AP Approver, Cash Exception Investigator, Cash Reconciliation Reviewer, Prepaid Preparer, Prepaid Reviewer | `skills/prior-period-precedent/SKILL.md` | New | ap-exception-investigation |
