...
def run_eval_modes(argv: list[str] | None = None) -> int:
    from simulations.stripe.modes import format_modes_table, run_three_modes, write_modes_report

    dest = DEFAULT_RUNS / "stripe_simulation"
    dest.mkdir(parents=True, exist_ok=True)
    with isolated_simulation(dest / "modes-state"):
        payload = run_three_modes()
    write_modes_report(payload, dest / "modes.json")
    print(format_modes_table(payload))
    print(f"\nWrote {dest / 'modes.json'}")
    return 0
...
def run_eval(argv: list[str] | None = None) -> int:
    ...
    with isolated_simulation(dest / "modes-state"):
        modes = run_three_modes()
    write_modes_report(modes, dest / "modes.json")
    print()
    print(format_modes_table(modes))
    return 0 if payload["passed"] else 1
