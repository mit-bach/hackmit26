...
def _explain_with_agent(explanation: VarianceExplanation, live: bool) -> VarianceExplanation:
    ...
    if not live:
        ...
        explanation.agents = [usage_from_agent(variance_analysis_agent)]
        return explanation
    try:
        from agent import run_agent
        ...
        explanation.agents = [usage_from_agent(agent)]
    except Exception:
        explanation.narrative = fallback()
        explanation.agents = [usage_from_agent(variance_analysis_agent)]
    return explanation
    ...
    *,
        ...
        try:
            ...
            snapshot = snapshot.model_copy(
                update={
                    ...
                    "used_agent": True,
                    "agents": [usage_from_agent(cash_forecast_agent)],
                }
            )
        except Exception:
            from reporting.agents import cash_forecast_agent

            snapshot = snapshot.model_copy(update={"agents": [usage_from_agent(cash_forecast_agent)]})
    else:
        from reporting.agents import cash_forecast_agent
    ...
    if live:
        try:
            ...
            fva = fva.model_copy(update={"narrative": result.narrative, "used_agent": True, "agents": [usage_from_agent(forecast_variance_agent)]})
        except Exception:
            from reporting.agents import forecast_variance_agent

            fva = fva.model_copy(update={"agents": [usage_from_agent(forecast_variance_agent)]})
    else:
        from reporting.agents import forecast_variance_agent
    ...
    if live:
        try:
            ...
            pack.agents = [usage_from_agent(board_reporting_agent)]
        except Exception:
            from reporting.agents import board_reporting_agent
